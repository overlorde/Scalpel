...
class Rewriter:
    ...
    def rewrite(src, rule_func=None):
        ...
        for unit in Walker:
            ...
            if not isinstance(new_stmts, list):
                raise Exception(
                    "The return type for rule_func function must be list type!"
                )
    ...
    def unused_stmt_insertion(self):
        inserted_stmt = ast.Expr(
            ast.Call(
                ast.Name("print", ast.Load()),
                [ast.Constant("this is an unused statement")],
                [],
            )
        )
    ...
    def for2while(self, node):
        if isinstance(node, ast.For) and isinstance(node.target, ast.Name):
            ...
            max_counter_init_stmt = ast.Assign(
                [max_counter_var], ast.Call(ast.Name("len"), [iter_object], [])
            )
...
class ASTRewriter(ast.NodeTransformer):
    ...
    def insert_before(self, loc=""):
        ...
        call_node = ast.Call(
            ast.Name(id="print", ctx=ast.Load()), [ast.Constant("testing", None)], []
        )
    ...
    def replace(self):
        ...
        if pos < 0:
            return self.ast
        call_node = ast.Call(
            ast.Name(id="print", ctx=ast.Load()), [ast.Constant("testing1", None)], []
        )
        new_stmt = ast.Expr(call_node)
        ...
        if len(node.targets) == 1 and isinstance(node.value, ast.Lambda):
            if isinstance(node.targets[0], ast.Name):
                ...
                return ast.FunctionDef(
                    fun_name, node.value.args, body_stmts, decorator_list
                )
        ...
        if len(node.targets) == 1 and isinstance(node.value, ast.ListComp):
            ...
            if len(ifs) == 0:
                append_attr = ast.Attribute(
                    value=ast.Name(id=new_lst_name, ctx=ast.Load()),
                    attr="append",
                    ctx=ast.Load(),
                )
